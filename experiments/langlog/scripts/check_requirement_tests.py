...
DOC_RE = re.compile(r"^\s*//=\s+([A-Za-z0-9_-]+\.md)#(.+?)\s*$")
...
def main() -> int:
    errors: list[str] = []
    seen_requirements: dict[tuple[str, str, str], tuple[Path, int, str]] = {}
    ...
    for path in RUST_FILES:
        ...
        for index, line in enumerate(lines):
            ...
            spec_refs = [
                DOC_RE.match(entry[1]).groups()
                for entry in block
                if DOC_RE.match(entry[1])
            ]
            ...
            for line_number, text in block:
                stripped = text.strip()
                if DOC_RE.match(stripped) or TYPE_RE.match(stripped) or QUOTE_RE.match(stripped):
            ...
            spec_doc, spec_anchor = spec_refs[0]
            key = (spec_doc, spec_anchor, quotes[0])
            ...
            if previous is not None:
                ...
                errors.append(
                    ...
                    f"{spec_doc}#{spec_anchor} / {quotes[0]!r}, already used by "
    ...
    for path in RUST_FILES:
        ...
        for index, line in enumerate(lines, start=1):
            if (path, index) in consumed_annotations:
                continue
            if DOC_RE.match(line) or TYPE_RE.match(line) or QUOTE_RE.match(line):
                errors.append(
